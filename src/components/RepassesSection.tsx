...
export default function RepassesSection({
  ...
  const handleExportCSV = () => {
    const header = ["Mes/Ano","Valor (R$)","Data Cadastro","Observações"].join(",");
    const rows = data.map(e => `"${e.mes_ano}","${e.valor}","${e.created_at}","${e.observacoes.replace(/"/g, '""')}"`).join("\n");
      ...
      <div className="relative z-10 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-blue-100 max-w-xl">
            Monitore os recursos transferidos mensalmente ao município de Vilhena sob o Termo de Cooperação SESAU. Transparência pública em tempo real.
          </p>
        </div>
        <button
